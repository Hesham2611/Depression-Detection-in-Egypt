...
def room(request, room_name, friend_id):
    ...
    consumers.detect(chats[chats.count()-1].text)
    return render(request, 'chat/chatroom.html', context)

def depressed_messages(request):
    if request.method == 'POST':
        message_text = request.POST.get('message_text')
        if is_depressed(message_text):
            DepressedMessage.objects.create(
                user=request.user,
                text=message_text,
                is_depressed=True
            )
    return render(request, 'depressed_messages.html')